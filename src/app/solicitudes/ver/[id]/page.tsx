...
import { useParams } from "next/navigation"; // Si usas Next.js para obtener los parámetros de la URL
...
export default function SolicitudDetalles() {
  ...
  const params = useParams(); // Obtener los parámetros de la URL (Next.js 13+)
  const { id } = params; // Asume que el id viene en la URL como `/solicitudes/ver/:id`
  ...
  useEffect(() => {
    const fetchSolicitud = async () => {
      try {
        ...
        router.push("/solicitudes"); // Redirigir a la lista si ocurre un error
      } finally {
        setLoading(false);
      }
    };
